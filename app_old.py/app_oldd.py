...
import pandas as pd
from services.production_service import calculate_production_risk, get_corrective_action
# =========================
...
# ============================================================
# SMART EXPLORATION RANKING
# ============================================================

if st.session_state.get("show_ranking", False):

    st.divider()
    st.subheader("🏆 Smart Exploration Ranking")

    # Check whether data has been uploaded and analyzed
    if "dataset" in st.session_state:

        ranking_data = st.session_state["dataset"].copy()

        # Check whether Exploration Score is available
        if "Exploration_Score" in ranking_data.columns:

            # Sort locations from highest to lowest exploration score
            ...
            ranking_data.insert(
                0,
                "Rank",
                range(1, len(ranking_data) + 1)
            )

            # Success message
            st.success(
                "Locations ranked successfully based on overall exploration potential!"
            )
            ...
            st.dataframe(
                ranking_data[display_columns],
                use_container_width=True,
                hide_index=True
            )

            # Top priority location
            st.markdown("### 🥇 Top Priority Location")

            top_location = ranking_data.iloc[0]
            ...
            with col1:
                st.metric(
                    "Location",
                    top_location.get("Location", "N/A")
                )

            with col2:
                st.metric(
                    "Exploration Score",
                    f"{top_location['Exploration_Score']:.2f}"
                )
            ...
            st.warning(
                "Please run Data Analysis first to generate Exploration Scores."
            )
    ...
    st.info(
        "Click 'View Exploration Ranking' to rank locations based on exploration potential."
    )
# =========================
# LOCATION ANALYSIS
# =========================
...
st.divider()
st.header("📉 Production Intelligence & Shortfall Prediction")

st.write(
    "Analyze production performance, operational constraints, and "
    "potential production shortfall risks."
)

if "dataset" not in st.session_state:

    st.info(
        "📁 Upload the SIH sample dataset using Data Analysis to "
        "view production intelligence."
    )

else:

    production_df = st.session_state["dataset"].copy()
    ...
    if all(column in production_df.columns for column in required_columns):
        ...
        st.success("✅ Production data is available for analysis!")

        selected_production_location = st.selectbox(
            "Select a location for production analysis:",
            production_df["Location"].dropna().unique(),
            key="production_location"
        )

        if st.button("Analyze Production Risk"):

            row = production_df[
                production_df["Location"] == selected_production_location
            ].iloc[0]

            expected = float(row["Production_Target_Tonnes"])
            actual = float(row["Actual_Production_Tonnes"])
            downtime = float(row["Equipment_Downtime_Hours"])
            rainfall = float(row["Rainfall_mm"])
            blasting_delay = float(row["Blasting_Delay_Hours"])

            # Convert rainfall into a simple weather risk score
            weather_risk = min((rainfall / 150) * 100, 100)

            shortfall, shortfall_percentage, risk_score, risk_level = (
            ...
            st.success(
                f"📊 Production analysis completed for "
                f"{selected_production_location}!"
            )
            ...
            with metric1:
                st.metric(
                    ...
                    f"{shortfall:,.0f} tonnes"
                )

            with metric2:
                st.metric(
                    "Shortfall %",
                    f"{shortfall_percentage}%"
                )
            ...
            st.subheader("⚠️ Operational Factors")
            ...
            with factor3:
                st.metric(
                    "Blasting Delay",
                    f"{blasting_delay} hours"
                )

            st.subheader("💡 AI-Recommended Corrective Actions")

            for action in actions:
                st.write(f"• {action}")

            st.caption(
                "This risk assessment is a decision-support prediction based "
                "on production, weather, equipment, and operational indicators."
            )

    else:

        st.warning(
            "⚠️ The uploaded dataset does not contain the production "
            "columns required for this analysis. Use sample_data.csv."
        )
# =========================
# FOOTER
# =========================
